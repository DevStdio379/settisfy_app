...
const MyRequests = ({ navigation, route }: MyRequestsScreenProps) => {
  ...
  const fetchData = async () => {
    if (user?.uid) {
      ...
      const activeJobs = setRequestsData.filter(booking => booking.status !== 6 && booking.status < 11);
      const inactiveJobs = setRequestsData.filter(booking => booking.status === 6 || booking.status >= 11);
